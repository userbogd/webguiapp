...
esp_err_t GetDeviceID(char *chip_id)
{
    ...
    BytesToStr((unsigned char *)id, (unsigned char *)chip_id, 4);
    return ESP_OK;
}
...
esp_err_t GetDeviceSerial(char *serial)
{
    ...
    snprintf(serial, 24, "%010u", (unsigned int)swap(d.Val));
    return ESP_OK;
}
...
esp_err_t SHA256Hash(unsigned char *data, int datalen, unsigned char *res)
{
...
esp_err_t SHA256hmacHash(unsigned char *data, int datalen, unsigned char *key, int keylen, unsigned char *res)
{
    mbedtls_md_context_t ctx;
    mbedtls_md_type_t md_type = MBEDTLS_MD_SHA256;
    mbedtls_md_init(&ctx);
    ...
    if (pxTaskStatusArray != NULL)
    {
        ...
        if (ulTotalRunTime > 0)
        {
            ...
            for (x = 0; x < uxArraySize; x++)
            {
                ...
                {
                    sprintf(pcWriteBuffer, "%-32s %-10llu %-10lu%% %d\r\n", pxTaskStatusArray[x].pcTaskName, (uint64_t)pxTaskStatusArray[x].ulRunTimeCounter, ulStatsAsPercentage,
                ...
                else
                {
                    ...
                    sprintf(pcWriteBuffer, "%-32s %-10llu <1%%  %d\r\n", pxTaskStatusArray[x].pcTaskName, (uint64_t)pxTaskStatusArray[x].ulRunTimeCounter,
                        (int)pxTaskStatusArray[x].usStackHighWaterMark);
                }
